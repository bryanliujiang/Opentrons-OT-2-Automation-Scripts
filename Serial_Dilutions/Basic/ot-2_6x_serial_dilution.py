...
""" 6X Serial Dilution: Instructions for OT-2

Procedure to run up to six instances of 6X serial dilutions per OT-2 unit.
"""

metadata = {
    'protocolName': '6X Serial Dilution',
    'apiLevel': '2.2'
    }
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    dilutionplate_6X_6 = None
    dilutionplate_6X_5 = None
    tiprack_6X_5 = None
    dilutionplate_6X_4 = None
    tiprack_6X_4 = None
    dilutionplate_6X_3 = None
    tiprack_6X_3 = None
    dilutionplate_6X_2 = None
    tiprack_6X_2 = None
    dilutionplate_6X_1 = None
    tiprack_6X_1 = None
    dilutionplate_6X = []
    tiprack_6X = []
    ...
    ************    ************
    ...
    class switch:
        def six():
            dilutionplate_6X_6 = protocol.load_labware(plate_type, 1)
            dilutionplate_6X.append(dilutionplate_6X_6)
            switch.five()
        def five():
            tiprack_6X_5 = protocol.load_labware(tiprack_type, 2)
            tiprack_6X.append(tiprack_6X_5)
            dilutionplate_6X_5 = protocol.load_labware(plate_type, 3)
            dilutionplate_6X.append(dilutionplate_6X_5)
            switch.four()
        def four():
            tiprack_6X_4 = protocol.load_labware(tiprack_type, 6)
            tiprack_6X.append(tiprack_6X_4)
            dilutionplate_6X_4 = protocol.load_labware(plate_type, 5)
            dilutionplate_6X.append(dilutionplate_6X_4)
            switch.three()
        def three():
            tiprack_6X_3 = protocol.load_labware(tiprack_type, 4)
            tiprack_6X.append(tiprack_6X_3)
            dilutionplate_6X_3 = protocol.load_labware(plate_type, 7)
            dilutionplate_6X.append(dilutionplate_6X_3)
            switch.two()
        def two():
            tiprack_6X_2 = protocol.load_labware(tiprack_type, 10)
            tiprack_6X.append(tiprack_6X_2)
            dilutionplate_6X_2 = protocol.load_labware(plate_type, 11)
            dilutionplate_6X.append(dilutionplate_6X_2)
            switch.one()
        def one():            	
            tiprack_6X_1 = protocol.load_labware(tiprack_type, 8)
            tiprack_6X.append(tiprack_6X_1)
            dilutionplate_6X_1 = protocol.load_labware(plate_type, 9)
            dilutionplate_6X.append(dilutionplate_6X_1)
        def default():
            sys.exit()
    ...
    * Drop the tip (implicitly into the trash at the back right of the robot’s 
    ...
    for i in range(instances):
        ...
        right_pipette.mix(repetitions, volume, 
                           dilutionplate_6X[i].columns()[0][0])
        right_pipette.drop_tip() 

        for j in range(filled_columns - 1): # indices start at zero
            right_pipette.transfer(volume, 
                                    dilutionplate_6X[i].columns()[j][0], 
                                    dilutionplate_6X[i].columns()[j+1][0], 
                                    new_tip='always', 
